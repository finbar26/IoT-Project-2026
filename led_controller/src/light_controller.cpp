// This code has different light modes as methods (all off, red, yellow, green, chase,
// rainbow, blink) that can be called by the website buttons. it also has methods for if the leds
// are being controlled manually by the button on the board or the button on the website

// Any serial prints can be removed they are only there as they helped me with debugging, so I
// left them incase you need them. I tried to comment it helpfully but just lmk if you are confused 
// :)
#include <Arduino.h>
...
void loop() {
  controlled_by_temp(initial_temp);
}